...
$query = "SELECT kd_karyawan FROM pegawai ORDER BY kd_karyawan DESC LIMIT 1";
$result = $conn->query($query);

if ($result->num_rows > 0) {
    // Ambil nomor RM terakhir
    $row = $result->fetch_assoc();
    $last_no_rkm_medis = $row['kd_karyawan'];
...
if (isset($_POST['simpan'])) {
    $kd_karyawan= $_POST['kd_pegawai'];
    $nama_karyawan   = $_POST['nama_karyawan'];
  


    if (empty($kd_karyawan) || empty($nama_karyawan)) {
        die("Data tidak lengkap atau tidak valid.");
    }

    $stmt = $conn->prepare("INSERT INTO pegawai (`kd_karyawan`,`nama_karyawan`) VALUES (?, ?)");

    if ($stmt === false) {
        die("Prepare failed: " . $conn->error);
    }

    $stmt->bind_param("ss", $kd_karyawan,  $nama_karyawan);
...
<div id="id01" class="modal">
  <form class="modal-content animate" action="../../modul/pegawai/index.php" method="post">
    <div class="container">
   <div class="inputform">

 <input type="text"  name="kd_pegawai" value="<?php echo $no_rkm_medis; ?>"  placeholder="No RM" required>
  <input type="text"  name="nama_pegawai" placeholder="Nama Karyawan" required>
   <div class="simpan">
...
$conn = new mysqli("localhost", "root", "", "simrs");
$result2 = $conn->query("SELECT * FROM pegawai");
$conn->close();
