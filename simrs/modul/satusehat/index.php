...
if (isset($_POST['simpan'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO satusehat (`client_key`,`secret_key`,`url`) VALUES (?,?,?)");

    if ($stmt === false) {
        die("Prepare failed: " . $conn->error);
    }

    $stmt->bind_param("sss", $client_key, $secret_key,  $url);
...
<div id="id01" class="modal">
  <form class="modal-content animate" action="../../modul/satusehat/index.php" method="post">
    <div class="container">
   <div class="inputform">
<input type="text"  name="secret_key" placeholder="Client Key" required>
 <input type="text"  name="client_key" placeholder="Secret Key" required>
...
// Query pertama
$result = $conn->query("SELECT * FROM satusehat");
...
$conn = new mysqli("localhost", "root", "", "simrs");
$result2 = $conn->query("SELECT * FROM satusehat");
$conn->close();
        ...
        <tbody>
            <?php if ($result && $result->num_rows > 0): ?>
                <?php while($row = $result->fetch_assoc()): ?>
                    <tr>
                      <td><?= htmlspecialchars($row["client_key"]) ?></td>
