...
if (isset($_POST['simpan'])) {
    $praktitioner_id= $_POST['praktitioner_id'];
    $kd_dokter    = $_POST['kd_dokter '];

  

    if (empty($praktitioner_id) || empty($kd_dokter)) {
        die("Data tidak lengkap atau tidak valid.");
    }

    $stmt = $conn->prepare("INSERT INTO satu_sehat_praktisi (`practitioner_id`,`kd_dokter`) VALUES (?, ?)");

    if ($stmt === false) {
        die("Prepare failed: " . $conn->error);
    }

    $stmt->bind_param("ss", $praktitioner_id,  $kd_dokter);
...
<html lang="en">
<head>
	<title>SATU SEHAT PRAKTISI</title>
	<meta charset="UTF-8">
...
<body>
  <div>
  <h1 class="h1title">SATU SEHAT PRAKTISI</h1>
</div>
...
<div id="id01" class="modal">
  <form class="modal-content animate" action="../../modul/satusehat/mapingpraktisi/index.php" method="post">
    <div class="container">
   <div class="inputform">
    
<div>
<label for="stts">Dokter</label><br>
        <select id="kd_dokter" name="nama_dokter" required>
...
// Query pertama
$result = $conn->query("SELECT * FROM satu_sehat_praktisi");
...
$conn = new mysqli("localhost", "root", "", "simrs");
$result2 = $conn->query("SELECT * FROM satu_sehat_praktisi");
$conn->close();
        ...
        <tbody>
            <?php if ($result && $result->num_rows > 0): ?>
                <?php while($row = $result->fetch_assoc()): ?>
                    <tr>
                        <td><?= htmlspecialchars($row["practitioner_id"]) ?></td>
