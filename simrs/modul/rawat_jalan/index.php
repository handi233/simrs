...
if (isset($_POST['simpan'])) {
    ...
    $kd_pj    = $_POST['kd_pj'];
    $kd_stts    = $_POST['kd_stts'];
  

    $errors = [];
    if (empty($no_rkm_medis) || empty($tgl_periksa) || empty($jam_periksa) || empty($kd_dokter) || empty($kd_pj) || empty($kd_stts)) {
        die("Data tidak lengkap atau tidak valid.");
    }

    // Pastikan format tanggal valid
    if (!preg_match("/^\d{4}-\d{2}-\d{2}$/", $tgl_periksa)) {
        die("Format tanggal tidak valid.");
    }

    // Pastikan format jam valid (contoh: 14:30:00)
    if (!preg_match("/^\d{2}:\d{2}:\d{2}$/", $jam_periksa)) {
    ...
    $stmt = $conn->prepare("INSERT INTO rajal (no_rkm_medis, tgl_periksa, jam_periksa, kd_dokter, kd_pj, kd_stts) VALUES (?, ?, ?, ?, ?, ?)");
    ...
    // Binding parameter
    $stmt->bind_param("ssssss", $no_rkm_medis, $tgl_periksa, $jam_periksa, $kd_dokter, $kd_pj, $kd_stts);
...
<div>
 ...
 <input type="text"  name="no_rkm_medis"  placeholder="No RM" required>

  <input type="date"  name="tgl_periksa"  required>

  <input type="time"  name="jam_periksa"  required>
</div>
     <label for="kd_dokter" >Dokter</label><br>
        <select id="kd_dokter" name="kd_dokter" required>
         ...
         <label for="kd_pj">Bayar</label><br>
        <select id="kd_pj" name="kd_pj" required>
         ...
         <label for="kd_stts">Status Daftar</label><br>
        <select id="kode_stts" name="kd_stts" required>
            ...
            // Menampilkan pilihan kategori
            while ($row = $result->fetch_assoc()) {
                echo "<option value='" . $row['kd_stts'] . "'>" . $row['nama'] . "</option>";
            }
                ...
                <?php while($row = $result->fetch_assoc()): ?>
                    <tr>
                        ...
                        <td><?= htmlspecialchars($row["jam_periksa"]) ?></td>
                           ...
                           <td><?= htmlspecialchars($row["kd_pj"]) ?></td>
                              <td><?= htmlspecialchars($row["kd_stts"]) ?></td>
                    </tr>
                <?php endwhile; ?>
            <?php else: ?>
