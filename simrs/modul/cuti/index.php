...
$query = "SELECT kd_karyawan FROM pengajuan_cuti ORDER BY kd_karyawan DESC LIMIT 1";
$result = $conn->query($query);

if ($result->num_rows > 0) {
    $row = $result->fetch_assoc();
    $last_kd_karyawan = $row['kd_karyawan'];
    $no_urutan = substr($last_kd_karyawan, -4); 
    $new_no_urutan = str_pad((int)$no_urutan +1 , 4, "0", STR_PAD_LEFT); 

    $kd_karyawan = "P" . $new_no_urutan;
} else {
    $new_kd_karyawan = "P0001";
}
...
if (isset($_POST['simpan'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO pengajuan_cuti (`kd_karyawan`,`tgl_mulai`,`tgl_selesai`,`alasan`) VALUES (?, ?,?,?)");

    if ($stmt === false) {
        die("Prepare failed: " . $conn->error);
    }

    $stmt->bind_param("ssss", $kd_karyawan,$tgl_mulai,$tgl_selesai,$alasan);
...
if ($result->num_rows > 0) {
    ...
    $no_kd_karyawan = "" . $new_no_urutan;
} else {
    // Jika belum ada data, buat nomor RM pertama
    $new_kd_karyawan = "0001";
}
...
<body>
  <div>
  <h1 class="h1title">PENGAJUAN CUTI</h1>
</div>
...
<div id="id01" class="modal">
  <form class="modal-content animate" action="../../modul/cuti/index.php" method="post">
    <div class="container">
   <div class="inputform">
<input type="text"  name="kd_karyawan" value="<?php echo $kd_karyawan?>" placeholder="Kd Karyawan" required>
<label>Tgl Mulai</label>
 <input type="date"  name="tgl_mulai"  placeholder="Tgl Mulai" required>
 <label>Tgl Selesai</label>
  <input type="date"  name="tgl_selesai" placeholder="Tgl Selesai" required>
...
// Query pertama
$result = $conn->query("SELECT * FROM pengajuan_cuti");
...
$conn = new mysqli("localhost", "root", "", "simrs");
$result2 = $conn->query("SELECT * FROM pengajuan_cuti");
$conn->close();
        ...
        <tbody>
            <?php if ($result && $result->num_rows > 0): ?>
                <?php while($row = $result->fetch_assoc()): ?>
                    <tr>
                        <td><?= htmlspecialchars($row["kd_karyawan"]) ?></td>
