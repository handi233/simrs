...
if (isset($_POST['simpan'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO departemen (`dep_id`,`nama_dep`) VALUES (?, ?)");
    $stmt->bind_param("is", $dep_id, $nama_dep); 
    $stmt->execute();
    
    if ($stmt === false) {
        die("Prepare failed: " . $conn->error);
    }
        $stmt->bind_param("ss", $dep_id, $nama_dep);
...
<body>
  <div>
  <h1 class="h1title">DEPARTEMENT</h1>
</div>
...
<div id="id01" class="modal">
  <form class="modal-content animate" action="../../modul/departemen/index.php" method="post">
    <div class="container">
   <div class="inputform">

 <input type="text"  name="dep_id" value=""  placeholder="Nomer Departemen" required>
  <input type="text"  name="nama_dep" placeholder="Nama Departemen" required>
   <div class="simpan">
...
$conn = new mysqli("localhost", "root", "", "simrs");
$result2 = $conn->query("SELECT * FROM departemen");
$conn->close();
            ...
            <?php if ($result && $result->num_rows > 0): ?>
                <?php while($row = $result->fetch_assoc()): ?>
                    <tr>
                        ...
                        <td><?= htmlspecialchars($row["nama_dep"]) ?></td>
