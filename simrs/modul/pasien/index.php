...
$query = "SELECT no_rkm_medis FROM pasien ORDER BY no_rkm_medis DESC LIMIT 1";
...
if (isset($_POST['simpan'])) {
    ...
    if (empty($no_rkm_medis) || empty($no_ktp) || empty($nama) || empty($alamat)|| empty($jk) || empty($tmp_lahir) || empty($tmp_lahir)) {
        die("Data tidak lengkap atau tidak valid.");
    }

    $stmt = $conn->prepare("INSERT INTO pasien (`no_rkm_medis`,`no_ktp`,`nama`,`alamat`,`jk`,`tmp_lahir`,`tgl_lahir`) VALUES (?,?,?, ?,?,?,?)");

    if ($stmt === false) {
        die("Prepare failed: " . $conn->error);
    }

    $stmt->bind_param("sssssss", $no_rkm_medis,  $no_ktp,$nama,$alamat,$jk,$tmp_lahir,$tgl_lahir);
...
<div id="id01" class="modal">
  <form class="modal-content animate" action="../../modul/pasien/index.php" method="post">
    <div class="container">
   <div class="inputform">
...
<div>
 ...
 <label for="gender" class="mt-2">Jenis Kelamin:</label>
    <select id="gender" name="jk" required>
        <option value="L">Laki-laki</option>
        <option value="P">Perempuan</option>
        <option value="Other">Lainnya</option>
    </select>
     <input type="text" class="mt-2" name="tmp_lahir"  placeholder="Tempat Lahir" required>
...
$conn = new mysqli("localhost", "root", "", "simrs");
$result2 = $conn->query("SELECT * FROM pasien");
$conn->close();
...
<div style="overflow-x: auto;">
   <div class="table-scroll">
    <h2 class="mb-4">Tabel Data </h2>
       <a href="../../modul/pasien/cetak.php"  target="_blank" class="btn btn-warning">CETAK</a>
