...
if (isset($_POST['simpan'])) {
    ...
    if (empty($no_rkm_medis) || empty($no_ktp) || empty($nama) || empty($alamat)) {
        die("Data tidak lengkap atau tidak valid.");
    }

    $stmt = $conn->prepare("INSERT INTO pasien (`no_rkm_medis`,`no_ktp`,`nama`,`alamat`) VALUES (?, ?,?,?)");

    if ($stmt === false) {
        die("Prepare failed: " . $conn->error);
    }

    $stmt->bind_param("ssss", $no_rkm_medis,  $no_ktp,$nama,$alamat);
