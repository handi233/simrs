...
if (isset($_POST['simpan'])) {
    ...
    $jam_inap    = $_POST['jam_inap'];
  


    if (empty($no_rkm_medis) || empty($tgl_inap) || empty($jam_inap)) {
        die("Data tidak lengkap atau tidak valid.");
    }

    $stmt = $conn->prepare("INSERT INTO rawatinap (`no_rkm_medis`,`tgl_inap`,`jam_inap`) VALUES (?, ?,?)");

    if ($stmt === false) {
        die("Prepare failed: " . $conn->error);
    }

    $stmt->bind_param("sss", $no_rkm_medis,  $tgl_inap,$jam_inap);
...
$query = "SELECT no_rkm_medis FROM rawatinap ORDER BY no_rkm_medis DESC LIMIT 1";
...
<body>
  <div>
  <h1 class="h1title">Rawat Inap</h1>
</div>
...
<div id="id01" class="modal">
  <form class="modal-content animate" action="../../modul/igd/index.php" method="post">
    <div class="container">
   <div class="inputform">
...
<div>
 ...
 <input type="text"  name="no_rkm_medis" value="<?php echo$no_rkm_medis ?>"  placeholder="No RM" required>
   ...
   <label>TGL INAP</label>
 <input type="date"name="tgl_inap" placeholder="Tgl Inap">
   </div>
    <label>JAM INAP</label>
 <input type="time"name="jam_inap" placeholder="Jam Inap">
</div>
...
// Query pertama
$result = $conn->query("SELECT * FROM rawatinap");
...
$conn = new mysqli("localhost", "root", "", "simrs");
$result2 = $conn->query("SELECT * FROM rawatinao");
$conn->close();
            ...
            <?php if ($result && $result->num_rows > 0): ?>
                <?php while($row = $result->fetch_assoc()): ?>
                    <tr>
                        ...
                        <td><?= htmlspecialchars($row["tgl_inap"]) ?></td>
                        <td><?= htmlspecialchars($row["jam_inap"]) ?></td>
                    </tr>
                <?php endwhile; ?>
            <?php else: ?>
