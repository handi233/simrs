...
if ($result->num_rows > 0) {
    ...
    $no_urutan = substr($last_no_rawat, -4); // Ambil 4 digit terakhir
...
// Query pertama
$result = $conn->query("SELECT * FROM biling");
...
$conn = new mysqli("localhost", "root", "", "simrs");
$result2 = $conn->query("SELECT * FROM biling");
  ...
  <div class="mt-3">
        ...
        <select id="status" name="status" required>
            ...
            while ($row = $result->fetch_assoc()) {
                echo "<option value='" . $row['kd_stts'] . "'>" . $row['nama_stts'] . "</option>";
            }
