...
$query = "SELECT kd_dokter FROM dokter ORDER BY kd_dokter DESC LIMIT 1";
$result = $conn->query($query);

if ($result->num_rows > 0) {
    // Ambil nomor RM terakhir
    $row = $result->fetch_assoc();
    $last_no_rkm_medis = $row['kd_dokter'];
...
if (isset($_POST['simpan'])) {
    ...
    if (empty($kd_dokter) || empty($nama_dokter)) {
        die("Data tidak lengkap atau tidak valid.");
    }

    $stmt = $conn->prepare("INSERT INTO dokter (`kd_dokter`,`nama_dokter`) VALUES (?, ?)");

    if ($stmt === false) {
        die("Prepare failed: " . $conn->error);
    }

    $stmt->bind_param("ss", $kd_dokter,  $nama_dokter);
...
<div id="id01" class="modal">
  <form class="modal-content animate" action="../../modul/dokter/index.php" method="post">
    <div class="container">
   <div class="inputform">

 <input type="text"  name="kd_dokter" value="<?php echo $no_rkm_medis; ?>"  placeholder="No RM" required>
...
$conn = new mysqli("localhost", "root", "", "simrs");
$result2 = $conn->query("SELECT * FROM dokter");
$conn->close();
