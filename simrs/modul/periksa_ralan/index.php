...
if (isset($_POST['simpan'])) {
    ...
    if (empty($no_rkm_medis) || empty($keluhan) || empty($diagnosa)) {
        die("Data tidak lengkap atau tidak valid.");
    }

    $stmt = $conn->prepare("INSERT INTO periksarajal (`no_rkm_medis`,`keluhan`,`diagnosa`) VALUES (?, ?,?)");

    if ($stmt === false) {
        die("Prepare failed: " . $conn->error);
    }

    $stmt->bind_param("sss", $no_rkm_medis,  $keluhan,$diagnosa);
...
<body>
  <div>
  <h1 class="h1title">Periksa Rajal</h1>
</div>
...
<div id="id01" class="modal">
  <form class="modal-content animate" action="../../modul/periksa_ralan/index.php" method="post">
    <div class="container">
   <div class="inputform">
...
// Query pertama
$result = $conn->query("SELECT * FROM periksarajal");
...
$conn = new mysqli("localhost", "root", "", "simrs");
$result2 = $conn->query("SELECT * FROM periksarajal");
$conn->close();
